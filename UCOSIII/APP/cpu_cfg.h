...
*/
                                                                ...
                                                                /*是否使用CPU 名字：DEF_ENABLED 或者DEF_DISABLED Configure CPU host name feature (see Note #1) :      */
#define  CPU_CFG_NAME_EN                        DEF_ENABLED
                                                                /*   DEF_DISABLED  CPU host name DISABLED               */
                                                                /*   DEF_ENABLED   CPU host name ENABLED                */

                                                                /*CPU 名字大小（ASCII 字符串形式） Configure CPU host name ASCII string size ...        */
...
*/
                                                                ...
                                                                /*CPU 时间戳功能配置 （只能选择其中一个） Configure CPU timestamp features (see Note #1) :     */
#define  CPU_CFG_TS_32_EN                       DEF_ENABLED/*是否使用32 位的时间戳变量*/
#define  CPU_CFG_TS_64_EN                       DEF_DISABLED/*是否使用64 位的时间戳变量*/
                                                                ...
                                                                /*配置CPU 时间戳计时器字大小 Configure CPU timestamp timer word size ...          */
                                                                /* ... (see Note #2) :                                  */
...
#if 0                                                           /*是否使用测量CPU 禁用中断的时间Configure CPU interrupts disabled time ...           */
#define  CPU_CFG_INT_DIS_MEAS_EN                                /* ... measurements feature (see Note #1a).             */
#endif

                                                                /*配置测量的次数 Configure number of interrupts disabled overhead ... */
...
#if 1                                                           /*是否使用CPU 前导零指令（需要硬件支持，在stm32 我们可以使用这个指令） Configure CPU count leading zeros bits ...           */
