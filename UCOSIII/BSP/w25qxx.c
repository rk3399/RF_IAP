...
#define W25QXX_TYPE  W25Q128	//默认是W25Q64
...
#define W25QXX_SPI_Init()	 SPI1_Init()		   			//初始化SPI
...
{  
	...
	byte=W25QXX_ReadWrite_byte(0);             //读取一个字节  
	W25QXX_CS=1;                            //取消片选     
	return byte;   
} 
//写W25QXX状态寄存器
...
static u16 W25QXX_ReadID(void)
{
	u16 Temp = 0;	  
	W25QXX_CS=0;				    
	W25QXX_ReadWrite_byte(0x90);//发送读取ID命令	    
...
{ 
    ...
    W25QXX_ReadWrite_byte((u8)((ReadAddr)>>16));  //发送24bit地址    
    W25QXX_ReadWrite_byte((u8)((ReadAddr)>>8));   
    W25QXX_ReadWrite_byte((u8)ReadAddr);   
    for(i=0;i<NumByteToRead;i++)
	{ 
        pBuffer[i]=W25QXX_ReadWrite_byte(0);   //循环读数  
    }
	W25QXX_CS=1;  				    	      
}  
...
static void W25QXX_Write_Page(const u8* pBuffer,u32 WriteAddr,u16 NumByteToWrite)
{
    ...
    W25QXX_ReadWrite_byte((u8)((WriteAddr)>>16)); //发送24bit地址    
    W25QXX_ReadWrite_byte((u8)((WriteAddr)>>8));   
    W25QXX_ReadWrite_byte((u8)WriteAddr);   
    for(i=0;i<NumByteToWrite;i++)
		W25QXX_ReadWrite_byte(pBuffer[i]);//循环写数  
	W25QXX_CS=1;                            //取消片选 
...
// 	printf("fe:%x\r\n",Dst_Addr);	  
    ...
    W25QXX_ReadWrite_byte((u8)((Dst_Addr)>>16));  //发送24bit地址    
